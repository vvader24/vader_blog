...
In the case of the loading matrices, we want to obtain values that maximize the cost in $\Sigma_mC_{i,j}$ as we want values that represent the highest similarity between the solutions from Philippines and Germany. The `HungarianSolver` function from the `RcppHungarian` package in R solves the *Minimum cost bipartite matching problem* using the Hungarian algorithm and can be modified to also solve the *Maximum cost bipartite matching problem*. It provides pairings of variables from the congruence coefficient matrics
...
Visualizing the data above aids in identifying the most robust model(s).The most efficient way of doing this is the line graph. \## Visualization
